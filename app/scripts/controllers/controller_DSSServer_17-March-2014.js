...
app.controller('NPICtrl', function ($rootScope, $scope, $http, $timeout, ISUrlService) {
    ...
    $scope.searchByNpiID = function () {
        ...
        NProgress.start();
        $http({
            url: 'http://esb.medigy.io/api/npidetails.json?npi=' + $scope.npiid,
            method: "GET",
        }).success(function (data) {
            if(data.details) {
                $scope.formdata = data.details.professional;
                ...
                $timeout(function () {
                    noty({
                        text: '<strong>No user found with the id ' + $scope.npiid + '</strong>',
                        type: 'error',
                        timeout: 5000
                    });
                }, 1000);
    ...
    $scope.searchByName = function () {
        ...
        NProgress.start();
        $http({
            url: ISUrlService.MEDIGY_ESB_URL + '/api/namedetails.json?name=' + $scope.npiname,
            method: "GET",
        }).success(function (data) {
            $scope.userdatas = true;
            if(data.details) {
    ...
    $scope.createProfessional = function (event) {
        ...
        var data = {
            ...
            "password": $scope.formdata.ProviderFirstName,
            "wso2Extension": {
                "professional": {
                    "proffname": $scope.formdata.ProviderFirstName,
                    "profmname": $scope.formdata.ProviderMiddleName,
                    "proflname": $scope.formdata.ProviderLastName,
                    "profgeneder": $scope.formdata.ProviderGenderCode,
                    "profstate": $scope.formdata.ProviderBusinessMailingAddressStateName,
                    "profPO": $scope.formdata.ProviderBusinessMailingAddressPostalCode,
                    "profphonrno": $scope.formdata.ProviderBusinessMailingAddressTelephoneNumber,
                    "proffaxno": $scope.formdata.ProviderBusinessMailingAddressFaxNumber
                }
            }
        };
        $http({
            ...
            if (data.Errors[0].code === '409') {
                $scope.error = 'User with the name ' + $scope.formdata.ProviderFirstName + ' already exists in the system..';
            } else {
                $scope.error = 'Error occurred in creating professional.';
            }
            $timeout(function () {
